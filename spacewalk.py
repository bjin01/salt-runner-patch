...
def patch(server, target_system, delay):
    log.warning("OK, patch func in use with params: %s, %s" %(server,target_system))
    errata_id_list = []
    ...
    log.warning("system ID is: %s" % systemid[0]['id'])
    errata_list = client.system.getRelevantErrata(key, systemid[0]['id'])
    #errata_output = api(server, "system.getRelevantErrata", systemid[0]['id'])
    #log.warning("errata output is: %s" % errata_output['output'])
